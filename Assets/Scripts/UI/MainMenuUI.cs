...
namespace MadKnight.UI
{
    ...
    {
        [Header("Menu Components")] [SerializeField]
        ...
        private AudioClip introMusicClip; // Clip phát 1 lần đầu tiên

        [SerializeField]
        private AudioClip backgroundMusicClip; // Clip loop sau khi intro hết
        ...
        private void EnsureAudioManager()
        {
            ...
            {
                Debug.Log(
                    "[MainMenuUI] AudioManager not found, creating new instance...");
            ...
            Debug.Log(
                $"[MainMenuUI] Loaded settings - Music Volume: {settings.musicVolume:F2}");
            settings.ApplyAll();
        ...
        private IEnumerator FadeInSequence()
        {
            ...
            // Fade in Subtitle
            yield return StartCoroutine(FadeInText(subtitleText,
            ...
            {
                yield return StartCoroutine(FadeInCanvasGroup(menuButtonsGroup,
                    buttonsFadeInDuration));
            }
        }
        ...
        private IEnumerator FadeInMusic()
        {
            ...
            {
                elapsed += Time.deltaTime;
                float volume = Mathf.Lerp(0f, targetVolume,
        ...
        private IEnumerator PlayMusicSequence()
        {
            ...
            {
                elapsed += Time.deltaTime;
                introMusicSource.volume = Mathf.Lerp(0f, targetVolume,
                    elapsed / musicFadeInDuration);
                yield return null;
            }
            ...
            if (backgroundMusicClip != null)
            {
                ...
                while (elapsed < crossfadeDuration)
                {
                    ...
                    {
                        introMusicSource.volume =
                            Mathf.Lerp(targetVolume, 0f, t);
                    }
            ...
            else
            {
                ...
                while (elapsed < crossfadeDuration)
                {
                    ...
                    if (introMusicSource != null)
                    {
                        introMusicSource.volume = Mathf.Lerp(targetVolume, 0f,
        ...
        public void OnContinueClick()
        {
            ...
            try
            {
                ...
                Debug.Log(
                    "[MainMenuUI] Has save file, showing Load Game Panel...");

                // Hiển thị Load Game Panel với 11 slots
                if (loadGamePanel != null)
                {
                    Debug.Log(
                        "[MainMenuUI] LoadGamePanel reference found, calling Show()...");
                    loadGamePanel.Show();
            ...
            {
                Debug.LogError(
                    $"[MainMenuUI] ERROR in OnContinueClick: {e.Message}\n{e.StackTrace}");
            }
        }
        ...
        private IEnumerator TogglePanel(CanvasGroup panel)
        {
            ...
            {
                elapsed += Time.deltaTime;
                panel.alpha = Mathf.Lerp(startAlpha, targetAlpha,
                    elapsed / duration);
                yield return null;
            }
